...
void set_alarma(void){
	...
	uint8_t WakeUpMin = sTime.Minutes + 3;
	uint8_t WakeUpWeekDay = sDate.WeekDay;
	
	/* Configuramos los limites*/
	/* Si linea 175 y 176 usan RTC_FORMAT_BIN la comparacion se hace en decimal,
	...
	if(WakeUpMin > 59){
		...
		if(WakeUpHour > 23){
			WakeUpHour = 0;
			WakeUpWeekDay += WakeUpWeekDay; //Para que no se pierda cuando haya cambio de dia
			if(WakeUpWeekDay > 7){
  ...
  alarma.AlarmTime.Seconds = sTime.Seconds;
